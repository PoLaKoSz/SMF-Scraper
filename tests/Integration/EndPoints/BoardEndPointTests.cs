...
{
    class BoardEndPointTests : TestClassBase
    {
        private readonly HttpClientMock _httpClient;
        private readonly BoardEndPoint _endPoint;



        public BoardEndPointTests()
            : base("Metin2Hungary.net", "Board", "BlackStorm")
        {
            ...
            _endPoint = new BoardEndPoint(settings, _httpClient);
        }



        [Test]
        public async Task Can_Extract_Children_Boards()
        {
            ...
            Board board = await _endPoint.Info(new Board(-1, ""));
            var actual = board.ChildBoards;

            Assert.That(actual[0], Is.EqualTo(expected[0]));
        }

        [Test]
        public async Task Can_Extract_Topics_From_First_Page()
        {
            ...
            Board board = await _endPoint.Info(new Board(-1, ""));
            var actual = board.Topics;

            CollectionAssert.AreEqual(expected, actual);
        }

        [Test]
        public async Task Can_Extract_Topics_From_Second_Page()
        {
            var expected = new List<Topic>()
                {
            ...
            _httpClient.ResponseHTML = base.GetHtmlData("2018_04_25_02_NonAuthenticated");
